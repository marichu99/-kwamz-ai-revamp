...
DEFAULT_CHECK_INTERVAL_MINUTES = 6
...
def get_active_config_for_user(user_id):
    """Get the active fraud detection configuration for a specific user."""
    from app.model.config import Config
    config = Config.query.filter_by(user_id=user_id, is_active=True).first()
    if not config:
        # Return default values if no active config for this user
        return {
...
def run_periodic_fraud_check(self):
    ...
    This task is triggered every minute by Celery Beat. For each user, it checks
    their active config for the configured interval (periodic_check_interval_minutes).
    It uses per-user Redis keys to track the last run time.
    """
    app = create_app()
    ...
    with app.app_context():
        try:
            ...
            for user in users_with_transactions:
                try:
                    # Get this user's active config
                    user_config = get_active_config_for_user(user.id)
                    check_interval = user_config.get('periodic_check_interval_minutes', DEFAULT_CHECK_INTERVAL_MINUTES)

                    # Check last run time from Redis (per-user key)
                    redis_key = f'fraud_detection:last_periodic_check:{user.id}'
                    try:
                        redis_client = get_redis_client()
                        last_run_str = redis_client.get(redis_key)
                        ...
                        if last_run_str:
                            ...
                            if minutes_since_last_run < check_interval:
                                logger.info(
                                    f"[Task {self.request.id}] Skipping user {user.email} - only {minutes_since_last_run:.1f} min "
                                    f"since last run (interval: {check_interval} min)"
                                )
                                results.append({
                                    'user_id': user.id,
                                    'email': user.email,
                                    'status': 'skipped',
                                    'reason': f'Last run {minutes_since_last_run:.1f} min ago'
                                })
                                continue
                    except Exception as redis_error:
                        logger.warning(f"[Task {self.request.id}] Could not check Redis for user {user.email}: {str(redis_error)}")

                    # Update last run time in Redis
                    try:
                        redis_client = get_redis_client()
                        redis_client.set(redis_key, datetime.now().isoformat())
                        redis_client.expire(redis_key, check_interval * 60 * 2)
                    except Exception as redis_error:
                        logger.warning(f"[Task {self.request.id}] Could not update Redis for user {user.email}: {str(redis_error)}")
