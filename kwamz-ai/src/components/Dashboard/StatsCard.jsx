// StatsCard Component with Chevron Overlap for the Last Card

import { useState, useEffect } from "react";
import { ArrowUp, ArrowDown, ChevronDown } from "lucide-react";
const StatsCard = ({ stat, index, isLast }) => {
  ...
  return (
    <div
      className={`relative z-10 transform transition-all duration-500 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
        }`}>
      <div className='bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl rounded-2xl p-6 border 
    border-slate-200/50 dark:border-slate-700/50 hover:shadow-2xl hover:shadow-slate-200/30 
    dark:hover:shadow-slate-900/30 transition-all duration-300 group hover:-translate-y-1'>
